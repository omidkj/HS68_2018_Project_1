...
class FeatureRemover():
    ...
    #There are four methods in this class:
        ...
        3. colinear: Returns collinear columns with a correlation greater than a specified correlation threshold
        4. low_importance: Returns features that are ranked low importance in the Random Forest Regressor or Classifier
    #Notes:
         using the idea adapted from: https://github.com/WillKoehrsen/feature-selector/blob/master/Feature%20Selector%20Development.ipynb
         but coding with numpy instead of Pandas and using different techniques.
    """
    def __init__(self):
        ...
        RandomForestClassifier based on the target_type.
            ...
            Parameters:
                ...
                target: array, shape = (1, )
                        target can be binary if target_type is classification or continuous if target_type is regression
                target_type: string
                        defines the RandomForest method if target is continuous or binary. default value = "classification"
                importance_thresh: float between 0 and 1
                        Pre-defined threshold set by the user, default value = 1/number of the features
            -----------
            Returns:
                A ranked (sorted) list containing indices of fetaures  with importance value below importance_thresh
        """
